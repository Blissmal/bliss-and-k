...
async function signIn(fd: FormData) {
  "use server";
  redirect((await login(String(fd.get("password") ?? ""))) ? "/admin/overview" : "/admin/login?error=1");
}
...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <div className="relative z-10 grid min-h-screen place-items-center px-4">
      <form action={signIn} className="biz-card w-full max-w-sm space-y-4 rounded-[2rem] p-8">
        <h1 className="font-display text-4xl font-bold tracking-[-0.03em]">Admin sign in</h1>
